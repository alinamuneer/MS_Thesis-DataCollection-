...
for ob in scene.objects:
     if ob.type == 'CAMERA':
         ...
         df.to_csv('OGP_dataset_collection.csv', mode='a', index=False, header=False)


#just pasted here

#now augment the cloth_2 by moving it on the hanger towards left
#just pasted here
for i in range(5):
    bpy.context.scene.frame_set(0)
    bpy.data.objects['Cloth_2'].location[0]+=-0.1
    run_and_stop_animation()
    for desired_frame in range(40,110,10):
        render_and_dataCollection(desired_frame,'image-clothleft'+str(i*0.1)+'frame'+str(desired_frame))

#set it back to initial
bpy.context.scene.frame_set(0)
bpy.data.objects['Cloth_2'].location[0]+=0.5

#now rotate cloth on hanger
for i in range(5):
    bpy.context.scene.frame_set(0)
    bpy.data.objects['Cloth_2'].rotation_euler[1]+=-0.1
    run_and_stop_animation()
    for desired_frame in range(40,110,10):
        render_and_dataCollection(desired_frame,'image-clothRotationleft'+str(i*0.1)+'frame'+str(desired_frame))
...
for i in range(5):
    ...
    run_and_stop_animation()
    for desired_frame in range(40,110,10):
        render_and_dataCollection(desired_frame,'image-clothRotationRight'+str(i*0.1)+'frame'+str(desired_frame))
